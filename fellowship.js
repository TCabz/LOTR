...
const makeMiddleEarth = () => {
  ...
  for (var i=0; i < lands.length; i++) {
    const landArticle = $(`<article></article>`);
    console.log(lands[i] = h1);
    landArticle.append(`<h1>${land}</h1>`);
  }
};
...
const makeHobbits = () => {
  ...
  // 4. append the ul to the shire
    ...
    $('#lands').append(`<ul id="hobbits"></ul>`);
    for (let i = 0; i < hobbits.length; i++) {
      $('#hobbits').append(`<li>${hobbits[i]}</li>`);
      $('#hobbits').addClass('hobbit');
    }
};
...
const makeBaddies = () => {
  ...
  const baddies = $(`<ul id="baddies"></ul>`);
  // 2. give each of the baddies a class of "baddy"
  $('#baddies').addClass('baddy');
  // 3. remember to append the ul to Mordor
  $('#Mordor').append(`<li>${baddies[i]}</li>`);
  }
};
...
const makeBuddies = () => {
  ...
  $('#middle-earth').append(`<aside id="buddies"></aside>`);
  // 2. display an unordered list of buddies in the aside
  $('#buddies').append(`<ul id="buddies"></ul>`);
...
const forgeTheFellowShip = () => {
  ...
  const $fellowshiph = $('<h1>The Fellowship</h1>');
...
const theBalrog = () => {
  ...
  $('#buddies').children().eq(0).text('Gandalf the White');
  // 2. add a class "the-white" to this element
  $('#buddies').children().eq(0).addClass('the-white');
...
const hornOfGondor = () => {
  ...
  $('#buddies').children().eq(4).css('line-through');

  // 3. Tricky: Remove the Uruk-Hai from the Baddies on the page
  ('#baddies').children().eq(2).remove();
